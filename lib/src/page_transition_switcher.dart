...
/// An internal representation of a child widget subtree that, now or in the
/// past, was set on the [ArnaPageTransitionSwitcher.child] field and is now in
...
class _ChildEntry {
  ...
  @override
  String toString() {
    return 'ArnaPageTransitionSwitcherEntry#${shortHash(this)}($widgetChild)';
  }
}
...
///
/// When an [ArnaPageTransitionSwitcher]'s [child] is replaced, the new child's
/// [primaryAnimation] runs forward and the value of its [secondaryAnimation]
/// is usually fixed at 0.0. At the same time, the old child's
/// [secondaryAnimation] runs forward, and the value of its primaryAnimation is
/// usually fixed at 1.0.
///
/// The widget returned by the [ArnaPageTransitionSwitcherTransitionBuilder]
/// can incorporate both animations. It will use the primary animation to
/// define how its child appears, and the secondary animation to define how its
/// child disappears.
...
///
/// This is a variation of an [AnimatedSwitcher], but instead of using the same
/// transition for enter and exit, two separate transitions can be specified,
/// similar to how the enter and exit transitions of a [PageRoute] are defined.
///
...
/// the value of its [primaryAnimation] is usually fixed at 1.0. The new
/// child's [primaryAnimation] runs forward and the value of its
/// [secondaryAnimation] is usually fixed at 0.0. The widget returned by the
/// [transitionBuilder] can incorporate both animations. It will use the
/// primary animation to define how its child appears, and the secondary
/// animation to define how its child disappears. This is similar to the
...
///
/// The following example shows a [transitionBuilder] that slides out the old
/// child to the right (driven by the [secondaryAnimation]) while the new
...
/// more than one old child can exist and be transitioning out while the newest
/// one is transitioning in.
///
/// If the *new* child is the same widget type and key as the *old* child, but
/// with different parameters, then [ArnaPageTransitionSwitcher] will *not* do
/// atransition between them, since as far as the framework is concerned, they
...
/// transition.
///
/// The same key can be used for a new child as was used for an
/// already-outgoing child; the two will not be considered related.
/// For example, if a progress indicator with key A is first shown, then an
/// image with key B, then another progress indicator with key A again, all in
/// rapid succession, then the old progress indicator and the image will be
/// fading out while a new progress indicator is fading in.
///
/// ArnaPageTransitionSwitcher uses the [layoutBuilder] property to lay out the
...
class ArnaPageTransitionSwitcher extends StatefulWidget {
  ...
  ///
  /// The [duration], [reverse], and [transitionBuilder] parameters must not
  /// be null.
  ...
  /// return null.
  ///
  /// The default [ArnaPageTransitionSwitcherLayoutBuilder] used is
  ...
  /// ),
  /// ```
  /// See [ArnaPageTransitionSwitcherLayoutBuilder] for more information about
  ...
  ///
  /// This function is the default way for how the new and old child widgets are
  /// placed during the transition between the two widgets. All children are
  /// placed in a [Stack] that sizes itself to match the largest of the child or
  /// a previous child. The children are centered on each other.
  ///
  /// See [ArnaPageTransitionSwitcherTransitionBuilder] for more information on
